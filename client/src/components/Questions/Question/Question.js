...
const Question = ({ question, setCurrentId }) => {
	...
	return (
		<React.Fragment>
			<Card className={classes.qbackground}>
				...
				<Button className={classes.seeq} value="Search" onClick={questionClicked}>
					{' '}
					See Question
				</Button>
				...
				{showQuestion ? (
					<div>
						<Typography className={classes.ans} variant="h5">
							{question.question}
						</Typography>
						<div className={classes.overlay2}>
							<Button style={{ color: 'black' }} size="small" onClick={() => setCurrentId(question._id)}>
								<MoreHorizIcon fontSize="default" />
							</Button>
						</div>
						<Button className={classes.seeq} value="Search" onClick={answerClicked}>
							{' '}
							Check Answer
						</Button>
						{showResults ? (
